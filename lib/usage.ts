...
async function getTierLimits(plan: Plan) {
  ...
  return {
    ...
    receiptScans: parseInt(data.find(d => d.key === `${plan}_receipt_scan_limit`)?.value ?? String(defaults.receiptScans)),
...
export async function checkUsageLimit(
  ...
  const plan: Plan = (subscription?.status === 'active' && (subscription?.plan === 'pro' || subscription?.plan === 'team'))
...
export async function checkStorageQuota(userId: string): Promise<{
  ...
  const plan: Plan = (sub?.status === 'active' && (sub?.plan === 'pro' || sub?.plan === 'team'))
    ? sub.plan as Plan
    : 'free'
