...
const DEFAULT_TIERS: TierConfig = {
  ...
  pro: { priceMonthly: 49, features: ['unlimitedInvoices', 'unlimitedScans', 'noBranding'] },
  team: { priceMonthly: 99, features: ['everythingInPro', 'inviteMembers', 'sharedCalendar'] },
}
    ...
    fetch('/api/config/tiers')
      ...
      .then(data => {
        if (data) setTiers({
          ...
          pro: { priceMonthly: data.pro?.priceMonthly ?? 49, features: data.pro?.features ?? DEFAULT_TIERS.pro.features },
          team: { priceMonthly: data.team?.priceMonthly ?? 99, features: data.team?.features ?? DEFAULT_TIERS.team.features },
        })
      })
      .catch(() => {})
  }, [])
    ...
    >
      ...
      <motion.div variants={fadeIn} className="rounded-2xl border border-[#1a3a5c] bg-[#102544] p-6">
        ...
        <div className="flex items-baseline gap-1 mb-6">
          <span className="text-4xl font-bold text-white">{tiers.free.priceMonthly} kr</span>
      ...
      <motion.div variants={fadeIn} className="rounded-2xl border-2 border-[#F59E0B] bg-[#102544] p-6 relative">
        ...
        <div className="flex items-baseline gap-1 mb-6">
          <span className="text-4xl font-bold text-white">{tiers.pro.priceMonthly} kr</span>
          <span className="text-sm text-[#C7D2FE]/50">/mån</span>
        </div>
        <div className="space-y-3 mb-8">
      ...
      <motion.div variants={fadeIn} className="rounded-2xl border border-[#1a3a5c] bg-[#102544] p-6">
        ...
        <div className="flex items-baseline gap-1 mb-6">
          <span className="text-4xl font-bold text-white">{tiers.team.priceMonthly} kr</span>
          <span className="text-sm text-[#C7D2FE]/50">/mån</span>
        </div>
        <div className="space-y-3 mb-8">
