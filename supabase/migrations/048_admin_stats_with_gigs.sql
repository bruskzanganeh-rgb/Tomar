-- Add gig_count to admin_user_stats
DROP FUNCTION IF EXISTS admin_user_stats(uuid[]);
...
AS $$
  SELECT
    ...
    COALESCE(gi.cnt, 0) AS gig_count
  ...
  LEFT JOIN (SELECT gigs.user_id, count(*) AS cnt FROM gigs WHERE gigs.user_id = ANY(p_user_ids) GROUP BY gigs.user_id) gi ON gi.user_id = u.id;
$$;
